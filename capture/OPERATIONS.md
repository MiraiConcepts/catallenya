...
infrastructure now — `documents.triage.service` reads the same file (see
...
```bash
sudo systemctl start capture.triage.service      # drain incoming/ now
bash capture/scripts/capture.sweep.sh --dry-run  # preview the nightly sweep
bash capture/tests/run.sh                        # offline suite
