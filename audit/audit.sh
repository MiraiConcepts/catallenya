...
# in TRACKED FILES", which is a repo-wide claim. The name was in a tracked file
...
#
# The name it searches for is never written in this file. Until 2026-09-17 the
# pattern was a literal here and the laptop client was allowlisted, so the check
# itself published the name it guarded. It comes from AUDIT_TAILNET_NAME, else from
# TAILNET_DNS_NAME in the gitignored .env, with ".ts.net" dropped so a bare tailnet
# name is caught too. No name is a FAILURE, never a pass: a leak check with nothing
# to search for reports clean forever. The name is never echoed, since this log is
# 0664.
tailnet_name="${AUDIT_TAILNET_NAME:-}"
if [[ -z "$tailnet_name" && -r "${COMPOSE_DIR}/.env" ]]; then
  tailnet_name="$(sed -n 's/^TAILNET_DNS_NAME=//p' "${COMPOSE_DIR}/.env" | tail -n 1 | tr -d "\"'\r")"
fi
tailnet_name="${tailnet_name%.ts.net}"
if [[ -z "$tailnet_name" ]]; then
  echo "  ✗ Tailnet name unknown: set AUDIT_TAILNET_NAME, or TAILNET_DNS_NAME in .env"
  FAIL=1
else
  tailnet_hits="$(git -C "$COMPOSE_DIR" grep -l -i -F -e "$tailnet_name" -- . 2>/dev/null || true)"
  if [[ -n "$tailnet_hits" ]]; then
    echo "  ✗ Hardcoded tailnet name in tracked files:"
    printf '      %s\n' $tailnet_hits
    FAIL=1
  else
