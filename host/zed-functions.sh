#!/bin/sh
# VENDORED FROM THE HOST, WITH LOCAL CHANGES — /etc/zfs/zed.d/zed-functions.sh
#
# Derived from zfs-zed 2.2.2-0ubuntu9.4. This is a COPY of a file that lives on the
# host; nothing reads it from here. `sudo bash host/zed.install.sh` puts it in place.
#
# WHY A COPY AT ALL. ZED is the only thing that notices a dying disk, and its ntfy
# sender had four defects this repo had already fixed everywhere else (below). Fixing
# them means editing a file outside the repo, which is invisible to git and lost on a
# rebuild — so the file is tracked here, the same shape as
# tailscale/initramfs-hooks/.
#
# WHY THE WHOLE FILE rather than a patch. Restoring is `cp`, which is the one operation
# that has to work when you are rebuilding a box. A patch would need the upstream file
# to still match.
#
# WHY IT SURVIVES APT, verified rather than assumed: /etc/zfs/zed.d/zed-functions.sh is
# a dpkg CONFFILE, so dpkg prompts rather than overwriting; unattended-upgrades runs
# `--force-confold`, which keeps the local copy; and zfs-zed was actually upgraded on
# this box on 2025-09-20 (9.3 -> 9.4) with the existing local modifications intact.
# This is the OPPOSITE of the sanoid trap, where the units were under /usr and
# therefore not conffiles, which is exactly why apt replaced those silently.
#
# THE STALENESS THIS BUYS INSTEAD. dpkg keeping the local file means a NEW upstream
# zed-functions.sh is written beside it as .dpkg-dist and never applied. After a
# zfs-zed upgrade, diff them:
#
#     diff /etc/zfs/zed.d/zed-functions.sh{,.dpkg-dist}
#
# LOCAL CHANGES, all inside zed_notify_ntfy() and the zed_ntfy_title() above it:
#   - contract-shaped titles built from ZEVENT_*, unmapped events pass through
#   - CR/LF strip and length cap on the Title header (hdr_safe's job)
#   - curl -f            an HTTP error no longer returns 0 with the alert lost
#   - curl --max-time    a wedged ntfy no longer hangs the disk watchdog
#   - curl --data-raw    a body beginning "@" is no longer read as a filename
#   - curl stderr to the ZED log instead of /dev/null
#   - Priority: high removed
#
# The ntfy support itself was ALREADY a local addition on this box before any of this.
#
...
# <https://docs.ntfy.sh/publish/#access-tokens> (ZED_NTFY_ACCESS_TOKEN) represents an
...
#   2: not configured
#
# zed_ntfy_title <fallback> -> the contract-shaped title, or the fallback unchanged.
#
# LOCAL ADDITION (2026-08-22), not upstream. See the header of this file.
#
# Built from ZEVENT_*, which zed exports into the zedlet's environment, rather than by
# parsing the subject string — there are FIVE subject shapes across the zedlets
# ("ZFS <subclass> event for <pool> on <host>", "... error for ...", "ZFS device fault
# for pool ...", and two more), so a regex over the prose would be matching on wording
# that upstream is free to reword.
#
# The mapping is exhaustive for the four enabled *-notify.sh zedlets: statechange
# fires ONLY for FAULTED/DEGRADED/REMOVED/UNAVAIL (it exits 3 otherwise), and the
# other three each have one subclass. ANYTHING UNMAPPED FALLS THROUGH UNCHANGED, so a
# zedlet enabled later is verbose rather than blank.
#
# The subject may not repeat the topic, which here is `zpool` — and the pool is
# literally NAMED zpool, so the pool cannot be the subject. The EVENT is the subject
# instead. See ntfy/MESSAGES.md.
zed_ntfy_title()
{
    local state

    case "${ZEVENT_SUBCLASS}" in
        scrub_finish)    echo "Scrub: Finished" ;;
        resilver_finish) echo "Resilver: Finished" ;;
        data)            echo "Data: Errors Found" ;;
        statechange)
            state="${ZEVENT_VDEV_STATE_STR}"
            case "${state}" in
                FAULTED)  echo "Device: Faulted" ;;
                DEGRADED) echo "Device: Degraded" ;;
                REMOVED)  echo "Device: Removed" ;;
                UNAVAIL)  echo "Device: Unavailable" ;;
                *)        echo "$1" ;;
            esac
            ;;
        *) echo "$1" ;;
...
zed_notify_ntfy()
{
    ...
    # LOCAL: the contract-shaped title, then a CR/LF strip and a length cap — the same
    # guard ntfy/ntfy.lib.sh calls hdr_safe. This value reaches an HTTP HEADER, and a
    # newline in a header is a request splitter. Machine-generated today, but a
    # hostname is not ours to promise about.
    subject="$(zed_ntfy_title "${subject}" | tr -d '\r\n' | cut -c1-200)"

    # Send the POST request and check for errors.
    #
    # LOCAL, four changes from upstream, all of them defects this repo already fixed
    # elsewhere on 2026-08-19:
    #
    #   -f          without it an HTTP error (5xx, a future auth failure) returns 0 and
    #               the alert VANISHES while zed records success. On the topic that
    #               reports a dying disk, that is the worst possible default.
    #   --max-time  without it a wedged ntfy hangs zed, the daemon watching the disks.
    #   --data-raw  curl reads a -d value beginning with "@" as a FILENAME.
    #   2>&1        upstream sent curl's stderr to /dev/null, so -S's reason was lost;
    #               it now reaches the ZED log, which is the only record there is.
    #
    # Priority: high REMOVED. Nothing in this repo shouts — a priority with one legal
    # value is a slot waiting for someone to put `high` in it, and everything-shouts is
    # how a topic gets muted, after which the message that mattered is the first lost.
    ...
    if [ "${rv}" -ne 0 ]; then
        zed_log_err "ntfy curl exit=${rv}: ${msg_out}"
        return 1
    fi
    msg_err="$(echo "${msg_out}" \
        | sed -n -e 's/.*"errors" *:.*\[\(.*\)\].*/\1/p')"
