...
> The **`documents`** pipeline of [catallenya](https://github.com/carrein/catallenya),
> mirrored from `documents/`. Force-synced by CI — open issues and pull requests on
> the parent repo, not here. Everything inside is named `documents`: the directory,
> the systemd units, the ntfy topic.
