...
- **ZFS scrub**: `zpool.scrub.timer` (1st of the month) — note `/etc/cron.d/zfsutils-linux` ALSO scrubs on the second Sunday and its two lines lack the `[ ! -d /run/systemd/system ]` guard the sanoid and e2scrub entries carry, so the pool is scrubbed twice a month at 2.5h a pass. Pick one owner. The timer has no `OnFailure=`, and `zpool scrub` returns immediately anyway, so the unit exiting 0 says nothing about the scrub's outcome — ZED covers that separately on the `zpool` topic
