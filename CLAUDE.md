...
**`restic.check@subset` and `restic.check@data` are `Class=scheduled`, NOT `monitor`** — despite the name. `check@data` holds an exclusive repository lock for ~8h30m; under the monitor class's 10-minute timeout systemd would kill the yearly integrity pass a few minutes in, every year. Their per-instance `MaxAge`/`Freshness` live in `restic/check/instance-{subset,data}.conf`, because systemd ignores the `[X-Catallenya]` section entirely and therefore never expands `%i` inside it — a template cannot carry per-instance metadata.
