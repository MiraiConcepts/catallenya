...
**Locks — never `timeout N restic …`, use `timeout -s INT N restic …`.** restic 0.16.4 registers its cleanup handler for **SIGINT only**; SIGTERM (what `timeout` and `systemctl stop` send) kills it before it releases its repository lock. Upstream fixed this in 0.17.0 (Bugfix #4703), which Ubuntu 24.04 will never ship — apt is frozen at 0.16.4 in every pocket and `self-update` is patched out of the Debian build. A `timeout 25 … check --read-data-subset` probe run this way on 2026-08-07 orphaned an **exclusive** lock and killed three nights of backups. `backup.sh`, `check.sh` and `forget.sh` all now prepend `restic unlock` and pass `--retry-lock=30m`; the two cover different failures and neither substitutes for the other — **`unlock` deletes a DEAD lock, `--retry-lock` waits out a LIVE one.** A live run is never collected: restic refreshes its own lock every 5 min and self-aborts at 22.5 min if it cannot, so a running job's lock can never reach the 30 min staleness threshold. Note the staleness rule is age-**first** (`Stale()` returns true on age alone, before hostname or PID are consulted) — the PID probe only decides locks younger than 30 min, so PID reuse cannot strand an old lock.
