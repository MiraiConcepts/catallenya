...
- **Documents intake**: `documents.intake.timer` runs daily at 03:00 SGT (clear of the immich bake at 04:00), classifying and filing anything dropped at the root of `syncthing/data/master/documents` per its `FILING-SCHEME.md`. Notifies the `documents` ntfy topic only when something was filed, removed, flagged, or failed — silent when nothing arrived. Scope is **root files only**; the ~131 already-filed documents are never read or moved, only hashed for duplicate detection. Classification goes through the shared AI layer (`ai/scripts/ai.lib.sh`, `claude-opus-4-8`, two passes per document — classify then adversarial verify with a separate schema); it moved off the `claude -p` subscription on 2026-07-30, which retired the `--tools`/`--setting-sources` containment flags along with the four CLI traps they worked around, and gained in-run retry so a single 429 no longer costs a night. The model is held at 4-8 through that move on purpose — the 2026-07-18 accuracy battery measured the model, and changing model and transport together would measure neither. Design + the now-historical CLI traps in `.claude/plans/documents-nightly-intake{,-canon}.md`
