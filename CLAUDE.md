...
- **Capture accepted risks** (reviewed 2026-07-27, do not re-raise without new information):
  ...
  - *Multi-event screenshots propose only the soonest event.* A festival or lineup page yields one proposal, with `events_seen` and a "Soonest of N on this page" line so the subset is explicit. One notification per event was designed and rejected: it means an `events[]` schema, an indexed callback and N records per capture — rewriting the paths that produced the 23-hour and alt-inherits-`end_time` bugs — to serve a workflow already solved by cropping the screenshot first.
