...
# up to four tier files plus a human-readable summary:
...
img_a_count=0; img_b_count=0; img_union_count=0
...
SQL
  ...
  img_union_count="$(cat "${RUN_DIR}/tier-a-image.tsv" "${RUN_DIR}/tier-b-image.tsv" \
...
# ── Summary ───────────────────────────────────────────────────────────────
{
  ...
  if [[ "${TYPE}" == "image" || "${TYPE}" == "all" ]]; then
    ...
    printf '  %-50s %s\n' "Unique assets across both tiers"                 "${img_union_count} assets"
    echo
